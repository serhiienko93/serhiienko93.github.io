jQuery(function($){
  ...
  $('.calculation-select-item__tt').click(function(){
   if($(this).parent().is('.calculation-select-item--dropdown-opened')){
    $(this).parent().removeClass('calculation-select-item--dropdown-opened');
   }else{
    $(this).parent().addClass('calculation-select-item--dropdown-opened');
   }
  });


  $('.calculation-select__tt').click(function(){
   if($(this).parent().is('.calculation-select--opened')){
    $(this).parent().removeClass('calculation-select--opened');
   }else{
    $('.calculation-select').removeClass('calculation-select--opened');
    $(this).parent().addClass('calculation-select--opened');
   }
  });
...
$(document).bind('click touchstart', function(e) {
  ...
  if (! $clicked.parents().hasClass("calculation-select")){
    $(".calculation-select").removeClass("calculation-select--opened");
  }
});
    ...
    $('#datepicker').datepicker({
        ...
        monthNamesShort: ['Января', 'Февраля', 'Марта', 'Апреля', 'Мая', 'Июня', 'Июля', 'Августа', 'Сентября', 'Октября', 'Ноября', 'Декабряь'],
  ...
  $(window).on('load scroll',function(){ 
    if ($(this).scrollTop() >= '350') {
        $('.basket-cont').addClass("show");
    }
    else  {$('.basket-cont').removeClass("show");}
  });
