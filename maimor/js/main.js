jQuery(function($){
  ...
  $('.animate-appearance').viewportChecker({
      classToAdd: 'animate'
  });
...
// Scrool menu fixed
  ...
  $('.aside-tt').click(function(){
   if($(this).parent().is('.aside--opened')){
    $(this).parent().removeClass('aside--opened');
   }else{
    $(this).parent().addClass('aside--opened');
   }
  });
  ...
  $('.bouquet-composition__tt').click(function(){
   if($(this).parent().is('.bouquet-composition--opened')){
    $(this).parent().removeClass('bouquet-composition--opened');
   }else{
    $(this).parent().addClass('bouquet-composition--opened');
   }
  });
...
if (browserMinWidth > 1023) {
  $(".about__img-wrap").each(function(i, elem) {
    var
      matrix = $(elem),
      matrixChild = $(elem).find('.about-img');
    matrix.mousemove(function(e) {
        var 
          matHWidth = matrix.width() / 2,
          matHHeight = matrix.height() / 2,
          relativeX = matrix.offset().left,
          relativeY = matrix.offset().top,
          left =  (e.pageX - relativeX - matHWidth) / 30,
          top = (e.pageY - relativeY - matHHeight) / 30;
        matrixChild.css("-webkit-transform", "rotateX(" + -left + "deg) rotateY(" + top + "deg)");
        matrixChild.css("transform", "rotateX(" + -left + "deg) rotateY(" + top + "deg)");
        });
    matrix.hover(
      function() {
      },
      function() {
        matrixChild.css("-webkit-transform", "rotateX(0) rotateY(0)");
        matrixChild.css("transform", "rotateX(0) rotateY(0)");
      }
    );
  });
}
